...
function case_parameters(::Type{Val{:KDEMO_compact}})
    ...
    ini.build.divertors = :lower
    ini.build.n_first_wall_conformal_layers = 1
    ini.equilibrium.B0 = 8.2        # Updated from bt
    ini.equilibrium.R0 = 3.5       # Updated from rmajor
    ini.equilibrium.ϵ = 0.2857
    ini.equilibrium.κ = 2.0          # Updated from kappa
    ini.equilibrium.δ = 0.6          # Updated from triang
    ini.equilibrium.ip = 7.6e6    # Updated from Ip
    ini.equilibrium.xpoints = :lower
    ini.equilibrium.boundary_from = :scalars

    ini.core_profiles.ne_setting = :greenwald_fraction_ped
    ini.core_profiles.ne_value = 0.71   #0.71 for high performance
    ini.core_profiles.ne_shaping = 0.9
    ...
    if isempty(ini.general.dd.core_sources) || !scenario_sources
        ...
        ini.nb_unit[1].power_launched = 10e7  # 60MW 고정 전력 설정
        ini.nb_unit[1].beam_energy = 330e3
        ini.nb_unit[1].beam_mass = 2.0
        ini.nb_unit[1].offaxis = true
    else
    ...
    ini.requirements.tritium_breeding_ratio = 1.2
    ini.requirements.power_electric_net = 150e6 # as example
